...
def train_vae(model, train_loader, args, is_cvae=False):
    ...
    for batch in train_loader:
        images = tf.Variable(batch[0])
        
        with tf.GradientTape() as gg:
         
            if is_cvae == False: 
                reconstruced_img, mu, logvar = model(images)
          
            else: 
                labels = one_hot(batch[1], 10) # MNIST labels = 10
                reconstruced_img, mu, logvar = model(images, labels)

            batch_loss = loss_function(reconstruced_img, images, mu, logvar)
            total_loss = total_loss + batch_loss            

        gradients = gg.gradient(batch_loss, model.trainable_variables)
